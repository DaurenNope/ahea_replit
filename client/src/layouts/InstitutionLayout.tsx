...
const InstitutionLayout: React.FC<InstitutionLayoutProps> = ({ 
  ...
  const pageFullTitle = pageTitle 
    ? `${pageTitle} - ${t(`institutes.${id}.shortName`)} - AHEU` 
    : `${t(`institutes.${id}.shortName`)} - AHEU`;

  // Determine page description
  const pageDescription = description || t(`institutes.${id}.description`, 'A leading institution at Almaty Humanitarian Economic University.');
